 ...
 * </p>
 * <p>
 * <strong>Note that {@link AbstractExpiringMap} is not synchronized and is not
 ...
 * @since 4.0
 * @version $Id: ExpiringPassiveMap.java 1686855 2015-06-22 13:00:27Z tn $
 */
public abstract class AbstractExpiringMap<K, V> extends AbstractMapDecorator<K, V> implements Serializable
{
	...
	{
		if (timeUnit == null) { throw new NullPointerException("Time unit must not be null"); }
		return TimeUnit.MILLISECONDS.convert(timeToLive, timeUnit);
	}

	/** map used to manage expiration times for the actual map entries. */
	private final Map<K, Long>				expirationMap	= new HashMap<K, Long>();
	 ...
	 */
	public AbstractExpiringMap(final ExpirationPolicy<K, V> expiringPolicy)
	{
	 ...
	 */
	public AbstractExpiringMap(final ExpirationPolicy<K, V> expiringPolicy, final Map<K, V> map)
	{
		super(map);
		if (expiringPolicy == null) { throw new NullPointerException("Policy must not be null."); }
	 ...
	 *            ALWAYS expire.
	 */
	public AbstractExpiringMap(final long timeToLiveMillis)
	{
	 ...
	 *             if the map is null.
	 */
	public AbstractExpiringMap(final long timeToLiveMillis, final Map<K, V> map)
	{
	 ...
	 *             if the time unit is null.
	 */
	public AbstractExpiringMap(final long timeToLive, final TimeUnit timeUnit)
	{
	 ...
	 */
	public AbstractExpiringMap(final long timeToLive, final TimeUnit timeUnit, final Map<K, V> map)
	{
	 ...
	 *             if the map is null.
	 */
	public AbstractExpiringMap(final Map<K, V> map)
	{
		this(-1L, map);
	}
	 ...
	 */
	protected boolean isExpired(final long now, final Long expirationTimeObject)
	{
	 ...
	 */
	protected void removeAllExpired(final long now)
	{
		...
		while (iter.hasNext())
		{
			...
			if (isExpired(now, expirationEntry.getValue()))
			{
				...
				final K k = expirationEntry.getKey();
				final V v = super.remove(k);
	 ...
	 */
	protected void removeIfExpired(final Object key, final long now)
	{
	...
	protected abstract void notifyExpired(Object key, V value);
}
